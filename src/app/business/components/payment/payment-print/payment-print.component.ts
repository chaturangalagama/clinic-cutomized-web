...
import { DrugItem } from '../../../../app/util/objects/DrugItem';
import { PrintTemplateService } from '../../../services/print-template.service';
import { Doctor } from '../../../../app/util/objects/SpecialityByClinic';
...
export class PaymentPrintComponent implements OnInit {
  ...
  onBtnPrintLabelClicked() {
    this.apiCmsManagementService.searchLabel('DRUG_LABEL').subscribe(
      res => {
        ...
        const drugs = this.consultationInfo.patientReferralEntity.dispatchItemEntities.filter(item => {
    ...
    this.apiCmsManagementService.searchLabel('MEDICAL_CERTIFICATE').subscribe(
      res => {
        ...
        const consultDoctor = this.store
          .getDoctorList()
          .find(doctor => doctor.id === consultation.patientReferralEntity.consultation.doctorId);
  ...
  onBtnPrintVCClicked() {
    this.apiCmsManagementService.searchLabel('VACCINATION_CERTIFICATE').subscribe(
      res => {
        ...
        const vaccinations = this.consultationInfo.patientReferralEntity.dispatchItemEntities.filter(item => {
          const storeItem = this.store.chargeItemList
            .map(item => item.item)
            .find(storeItem => storeItem.id === item.itemId);
  ...
  patchMCValueToPrintFormGroup(mc) {
    ...
    while (medicalCertificateArray.length) {
      medicalCertificateArray.removeAt(0);
    }
  ...
  patchReferralValueToPrintFormGroup(referral) {
    ...
    while (printReferralArray.length) {
      printReferralArray.removeAt(0);
    }
