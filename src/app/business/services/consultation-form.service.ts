...
import { AtLeastOneFieldValidator } from '../../app/util/validators/AtLeastOneValidator';
...
export class ConsultationFormService {
  ...
  createConsultationFormGroup() {
    return this.fb.group({
      ...
      dispatchItemEntities: this.chargeFormService.createEmptyDispatchDetails()
    });
  }
  ...
  // DIAGNOSIS
  initDiagnosis() {
    const id = new FormControl(undefined, [Validators.required]);
    // const id = new FormControl(null, [Validators.required]);
  ...
  patchDispatchItemsToFormArray(refEntity, dispatchItemEntities: FormArray) {
    if (refEntity.dispatchItemEntities) {
      ...
      for (let i = 0; i < refEntity.dispatchItemEntities.length; i++) {
        ...
        if (item) {
          ...
          const dosgeInstruction = this.store
            .getDosageInstructions()
            .find(instruction => instruction.code === purchaseItem.dosageInstruction) || { code: '', instruct: '' };
    ...
    this.chargeFormService.chargeItemDetails = dispatchItemEntities;
  }
  ...
  isService(itemSelected){
    if (itemSelected) {
      console.log('isservice: ', itemSelected.itemType);
