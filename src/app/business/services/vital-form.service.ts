...
export class VitalFormService {
  ...
  generateFormGroup(patientId?: string, code?: string, value?: string) {
  ...
  initVitals() {
    ...
    // Remove BMI
    this.vitals = this.vitals.filter(data => data.code !== 'bmi');

    console.log(
      'TCL: VitalFormService -> initVitals -> this.store.vitalConfigurationsForAdd',
      this.store.vitalConfigurationsForAdd
    );
    console.log('TCL: VitalFormService -> initVitals -> this.vitals', this.vitals);
  }
  ...
  saveVital(patientId) {
    ...
    let bmiParamsCounter = 0;
    let weight = -1;
    let height = -1;

    vitalData = vitalData.filter(data => {
      if (data.code && data.value) {
        if (data.code === 'weight' || data.code === 'height') {
          bmiParamsCounter++;
          data.code === 'weight' ? (weight = data.value) : (height = data.value);
        }
        return true;
      }
      return false;
    ...
    // calculate BMI
    if (bmiParamsCounter === 2) {
      const bmi = this.calculateBmi(weight, height);
      vitalData.push({ patientId: patientId, code: 'bmi', value: bmi, comment: '' });
    }
