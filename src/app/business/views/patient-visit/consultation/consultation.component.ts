...
import { Component, OnInit, EventEmitter, Output, ViewChild, Input, AfterViewInit, OnDestroy } from '@angular/core';
...
export class ConsultationComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  patchValueToConsultationFormGroup(consultation) {
    const refEntity = consultation.patientReferralEntity;
    ...
    this.consultationFormService.patchDispatchItemsToFormArray(
      consultation.patientReferralEntity,
    ...
    this.consultationFormService.patchReferralToFormArray(consultation.patientReferralEntity, referralArray);
  ...
  copyPrescription(event) {
    const dispatchItemFormArray = this.visitManagementFormGroup
      .get('consultationFormGroup')
      .get('dispatchItemEntities') as FormArray;
    const copiedPrescriptionArray = new FormArray([]);

    this.consultationFormService.patchDispatchItemsToFormArray(event, copiedPrescriptionArray);
    copiedPrescriptionArray.controls.forEach( copiedPrescription =>{
      dispatchItemFormArray.push(copiedPrescription);
    });
  }
