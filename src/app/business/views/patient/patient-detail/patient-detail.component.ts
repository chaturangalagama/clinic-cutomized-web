...
// Objects
import { Clinic as ClinicOnly } from '../../../../app/util/objects/response/Clinic';
...
import { Page } from '../../../../app/util/model/page';
...
export class PatientDetailComponent implements OnInit, OnDestroy { //changed medicalReferenceEntity to patientReferralEntity
    ...
    this.patientDetailFormGroup.valueChanges
      ...
      .subscribe(values => {
        if (!values.isHistoryList && values.historyDetailIndex !== -1) {
          ...
          this.store.setPatientVisitRegistryId(history.visitId, true);

          if (!history.patientReferralEntity.consultation) {
            return;
          }
          const consultation = history.patientReferralEntity.consultation;
          ...
          if (consultation.patientReferral) {
            ...
            const patientReferrals = history.patientReferralEntity.patientReferral.patientReferrals;
          ...
          const certificates = history.patientReferralEntity.medicalCertificates;
          if (certificates) {
    ...
    this.apiPatientVisitService.getPatientVisitHistoryPage(this.store.getPatientId(), this.historyPage).subscribe(
      res => {
        this.histories = res.payload
          ...
          .sort((a, b) => {
            const momentA = moment(a.patientReferralEntity.consultation.consultationStartTime, DB_FULL_DATE_FORMAT);
            const momentB = moment(b.patientReferralEntity.consultation.consultationStartTime, DB_FULL_DATE_FORMAT);
  ...
  updateHistoryList() {
    ...
    if (histories && histories.length) {
      ...
      histories.forEach(history => {
        ...
        historyArray.push(
          this.fb.group({
            date: moment(consultation.consultationStartTime, DB_FULL_DATE_FORMAT).format(
              DISPLAY_DATE_TIME_NO_SECONDS_FORMAT
            ),
