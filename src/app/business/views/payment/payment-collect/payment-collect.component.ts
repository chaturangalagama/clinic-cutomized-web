...
export class PaymentCollectComponent implements OnInit {
  ...
  updateCollectChargeFormGroup() {
    ...
    this.collectFormGroup.get('collectChargeFormGroup').patchValue({
      otherCharge,
      otherChargeGst,
      cashRoundAdjustedValue: otherCharge + otherChargeGst - cashAdjustmentRounding
    });
  ...
  executePay() {
    ...
    if (!billArr.length) {
      ...
      if (bill.billMode === 'CASH') {
        bill.cashAdjustment = this.collectFormGroup.get('paymentFormGroup').get('cashRoundAdjustedValue').value || 0;
      }
    } else {
      billArr = billArr.filter(bill => bill.amount);
    }

    const bill = billArr.map((bill, index) => {
      return {
        // "caseId": this.store.getCaseId(),
        amount: bill.amount,
        billMode: bill.billMode,
        externalTransactionId: bill.externalTransactionId || '',
        cashAdjustment: bill.cashAdjustment || 0,
